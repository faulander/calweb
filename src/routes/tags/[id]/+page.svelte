...
<svelte:head>
	<title>{data.tag.name} — {m.app_name()}</title>
</svelte:head>

<div class="space-y-4">
	<a href={localizeHref('/tags')} class="inline-flex items-center gap-1 text-sm text-neutral-500 hover:text-neutral-700 dark:text-neutral-400 dark:hover:text-neutral-200">
	...
	<p class="text-sm text-neutral-500 dark:text-neutral-400">{bookCount(data.books.total)}</p>
